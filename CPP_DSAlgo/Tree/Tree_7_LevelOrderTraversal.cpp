...
void printNodeOnLevel(struct Node * p, int level)
{
    if(p == NULL)
        return;
    else if(level == 0)
    ...
    {
        printNodeOnLevel(p -> left, level - 1);
        printNodeOnLevel(p -> right, level - 1);
    }
}
...
int main()
{
	...
	cout << "\nLevel Order Traversal of this Binary Tree:\n";
